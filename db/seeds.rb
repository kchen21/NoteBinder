...
Note.create!(title: "Welcome to Notebinder!", body: "<p>Notebinder is an Evernote-inspired web application built using Ruby on Rails and React/Redux! You can use it to write notes and save them to a database! Click on the 'New Note' (+) icon on the left to create a new note!</p>", notebook_id: 1) # id: 1
Note.create!(title: "Notes", body: "<p>Your notes are all available in the Notes index, which is available for viewing whenever a note is being created or updated. You can click on the Notes icon in the sidebar to view just the Notes index itself.</p>", notebook_id: 1) # id: 2
...
Note.create!(title: "Updating", body: "<p>The contents of a note or notebook can be updated once it is created! Clicking on a note from an index will </p>", notebook_id: 1) # id: 5
Note.create!(title: "Deleting", body: "<p>Notes, notebooks, and tags can also be deleted. When updating a note or notebook or viewing each tag of the Tags index, note that there exists a Trash icon for deleting the item. Be careful when deleting a notebook; it'll delete all the notes in that notebook as well!</p>", notebook_id: 1) # id: 6
